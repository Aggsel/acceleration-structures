...
__global__ void normalize_output_image(Vec3 *raw_traversal_data, Vec3 *output_image, RenderConfig config, int custom_normalization){
  ...
  float inv_max_value = 1.0 / max_value;
  if(custom_normalization > 0)
    inv_max_value = 1.0 / custom_normalization;

  for (int i = 0; i < img_size; i++){
    float val = raw_traversal_data[i].x() * inv_max_value;
...
class Raytracer{
    ...
    Vec3* renderTraversalHeatmap(Camera cam, Node* bvh_root, Vec3** device_traversal_statistics, int custom_normalization){
      ...
      normalize_output_image<<<1,1>>>(*device_traversal_statistics, ptr_device_img, config, custom_normalization);
      checkCudaErrors(cudaDeviceSynchronize());
      return ptr_device_img;
    }
};